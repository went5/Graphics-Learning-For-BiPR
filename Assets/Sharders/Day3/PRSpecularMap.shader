    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                fixed3 specular = _LightColor0.rgb * specTex * pow(RdotV, _Shininess) * specTex;
                const fixed3 diffuse = _LightColor0 * NdotL * texColor;
                const fixed3 ambient = UNITY_LIGHTMODEL_AMBIENT * texColor;

                fixed4 color = fixed4(diffuse + specular + ambient, 1.0);
                return color;
            }
            ENDCG
        }
